...
public class OracleH2comp {
	...
	public void test() {
		...
		OptJoinFSP(myAW); // card 9760
		OptJoinFSP(myOAW);
		
		OptJoinFSP(myAW); // card 9760
		OptJoinFSP(myOAW);
		
		OptJoinSP(myAW); // card 9760
		OptJoinSP(myOAW);
		
		OptJoinFP(myAW); // card 9760
		OptJoinFP(myOAW);
		
		
		JoinGtoECS(myAW); // card 9760
		JoinGtoECS(myOAW);
		
		
		JoinGtoECSDiffJoinOrder(myAW); // card 9760
		JoinGtoECSDiffJoinOrder(myOAW);
		
		JoinGtoECSOR(myAW); // card 9760
		JoinGtoECSOR(myOAW);
		
		
		JoinGtoECSHalf(myAW);
		JoinGtoECSHalf(myOAW);
		

		JoinGtoECSHalfOR(myAW); // card 9760
		JoinGtoECSHalfOR(myOAW);
		
		JoinUCSaaECjoinECaaECS(myAW); // card 9760
		JoinUCSaaECjoinECaaECS(myOAW);
		

		JoinGaaUCjoinUCaaUCS(myAW); // card 9760
		JoinGaaUCjoinUCaaUCS(myOAW);
